...
function openProductForm(id){
  ...
  if(id){
    ...
    ['productId','productTitle','productDesc','productSalesLink','productLink'].forEach(id=> document.getElementById(id).value = '');
  }
}
...
function renderFreebieTable(){
  ...
  if(FREEBIES.length===0){ body.innerHTML = `<tr><td colspan="2">Belum ada materi gratis.</td></tr>`; return; }
...
function openFreebieForm(id){
  ...
  if(id){
    ...
    ['freebieId','freebieTitle','freebieDesc','freebieLink'].forEach(fid=> document.getElementById(fid).value = '');
  }
}
